...
def debug_state_dict_mismatch(model, model_state_dict):
    ...
    print("\n--- A. Keys in state_dict but NOT in model (多出来的权重) ---")
    for k in sorted(extra_in_state):
        print("   [+] ", k, "shape:", tuple(model_state_dict[k].shape))

    print("\n--- B. Keys in model but NOT in state_dict (缺少的权重) ---")
    for k in sorted(missing_in_state):
        print("   [-] ", k, "shape:", tuple(model.state_dict()[k].shape))

    # 2. 形状不匹配的参数
    print("\n--- C. Keys with different shape (维度不一致) ---")
    common_keys = model_keys & state_dict_keys
    for k in sorted(common_keys):
        t1 = tuple(model.state_dict()[k].shape)
        t2 = tuple(model_state_dict[k].shape)
        if t1 != t2:
            print(f"   [!] {k}: model_shape={t1}, state_shape={t2}")
...
def load_model_from_single_file(state_dict, model_names, model_classes, model_resource, torch_dtype, device):
    ...
    for model_name, model_class in zip(model_names, model_classes):
        ...
        if isinstance(state_dict_results, tuple):
            ...
            if extra_kwargs == {}:
                raise ValueError(f"extra_kwargs empty, please check the model {model_name} config.")
...
class ModelDetectorFromSingleFile:
    ...
    def match(self, file_path="", state_dict={}):
        ...
        print(f'keys_hash_with_shape: {keys_hash_with_shape}')
        # print(f'keys_hash_with_shape_dict: {self.keys_hash_with_shape_dict.keys()}')
        if keys_hash_with_shape in self.keys_hash_with_shape_dict:
            return True
        print('failing keys_hash_with_shape')
...
class ModelManager:
    ...
    def load_model(self, file_path, model_names=None, device=None, torch_dtype=None):
        ...
        if isinstance(file_path, list):
            print(f"  Loading from multiple files: {file_path}")
            # input('x')
            state_dict = {}
        ...
        for model_detector in self.model_detector:
            print(f"  Trying model detector: {model_detector.__class__.__name__}")
