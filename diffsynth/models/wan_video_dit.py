...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, c):
        # print(f'c shape in TransformerBlock: {c.shape}')
        for name, param in self.adaLN_modulation.named_parameters():
            if torch.isnan(param).any() or torch.isinf(param).any():
                print(f"⚠️ [NaN/Inf] adaLN_modulation param {name} contains NaN or Inf!")
